...
for jj=1:length(phiv)
    ...
    for kk=1:length(cv)
        kk
        %
        c = cv(kk);  b = sqrt(2*phi/m); ep =  2*c/(m*pi*b^2);  a = ep*b;
        if a>0.024
            abcd;
        end
        %
...
set(groot,'defaultLegendInterpreter','latex');
% 
plot(cv,Lamyv(:,1),'k','LineWidth',1.5)
hold on
plot(cv,Lamyv(:,2),'r','LineWidth',1.5)
plot(cv,Lamyv(:,3),'m','LineWidth',1.5)
plot(cv,Lamyv(:,4),'b','LineWidth',1.5)
hold on; box on
%
legend({'$\phi=0.4$','$\phi=0.7$','$\phi=1$','$\phi=1.3$'},...
